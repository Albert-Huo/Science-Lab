...
看板分为两层：默认的“网站使用概览”来自无身份聚合，只累计页面PV、实验打开、固定操作和四类来源；“安全流量”来自必要运行日志，用于防护、排障和自动访问分类。两条链路独立存储、独立权限，安全日志不得回填产品统计或反推访问路径。产品统计不设置Cookie、不创建访客ID、不做浏览器指纹，也不保存完整IP、UA、Referer、query或原始事件；后台明确显示“UV未采集”“会话未采集”“实验完成事件未接入”。

需要把行为关联到同一浏览器的第二层会话统计目前未启用。在未解决不满14周岁用户的监护人同意和专门规则前保持关闭，因此首页不会出现统计同意弹窗。完整边界见公开的 [`privacy.html`](privacy.html)。内置AI为额度与防滥用使用的24小时签名Cookie不属于产品统计，具体说明仍见下方AI章节。
...
“下载当前数据”导出安全流量快照，“下载产品统计”单独导出无身份聚合快照，两者不会混成一份用户轨迹。“下载历史CSV”提供最近400天已归档的安全流量每日汇总，首个自然日结束前暂不可用。CSV不包含原始IP、浏览器标识、查询参数或完整来源网址，小时和每日访客估算不能相加作为期间去重人数。
...
复用现有服务器、独立日志和Node运行时，无需购买外部统计服务，电脑不用一直开机。无身份产品统计使用限制为32MB的独立本地Redis直接聚合，不保存事件明细；安全日志沿用受控轮转，每日汇总另保留400天。历史汇总不是原始日志备份，超过日志留存期的中断数据无法补算。维护和部署说明见 `server/traffic/README.md`。
