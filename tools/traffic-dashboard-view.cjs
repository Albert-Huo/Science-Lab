...
function toCsv(data, scope = 'current') {
  ...
  const headers = ['记录类型', '区间开始_北京时间', '区间结束_北京时间', '生成时间_北京时间', '采集状态', '入口请求', '访客估算_不可跨行相加', '全部请求', '已识别自动请求', '4xx请求', '5xx请求', '电脑入口', '手机入口', '平板入口', '站内来源', '外部来源', '无来源信息'];
  ...
  const row = (kind, start, end, item, coverage) => {
    ...
    return [kind, format(start), format(end), format(data.generatedAt), coverage === 'unavailable' ? '未采集' : coverage === 'partial' ? '部分时段已采集' : '按日志统计', ...metrics];
  };
...
function renderDashboard(data) {
  if (data.schema !== 2 || data.hours.length !== 24 || !Array.isArray(data.history)) throw new Error('看板数据无效');
  const css = fs.readFileSync(path.join(__dirname, 'traffic-dashboard.css'), 'utf8');
...
<section class="panel trend-panel"><div class="panel-heading"><div><h2>小时趋势</h2><p>查看访问随时间的变化</p></div><div class="segmented" role="group" aria-label="趋势指标"><button type="button" data-metric="entryRequests" aria-pressed="true">入口请求</button><button type="button" data-metric="visitorEstimate" aria-pressed="false">访客估算</button><button type="button" data-metric="requests" aria-pressed="false">全部请求</button></div></div>
<div class="chart-topline"><span id="chart-detail" aria-live="polite">轻触柱形或用键盘选择，查看该小时数据</span><span id="chart-scale">最高 ${count(max === 1 && !total.entryRequests ? 0 : max)}</span></div><div class="chart"><div class="chart-guides" aria-hidden="true"></div><div class="bars">${bars}</div></div><div class="chart-footnote"><span><i class="legend"></i> <span id="metric-label">入口请求</span></span><span>小时访客估算不可相加为全天人数</span></div>
<details class="hour-details"><summary>查看 24 小时明细</summary><div class="table-scroll"><table><thead><tr><th>时段</th><th>入口请求</th><th>访客估算</th><th>全部请求</th></tr></thead><tbody>${data.hours.map(hour => `<tr><th>${escape(time(hour.start))} — ${escape(time(hour.end).slice(-5))}${hour.coverage !== 'recorded' ? '<small>' + (hour.coverage === 'unavailable' ? '未采集' : '部分采集') + '</small>' : ''}</th>${['entryRequests', 'visitorEstimate', 'requests'].map(key => `<td>${hour.coverage === 'unavailable' ? '—' : count(hour[key])}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details></section>
...
<details class="methodology"><summary>统计口径与数据说明</summary><div><p>入口请求只计算浏览器特征的 GET 首页请求，状态为 200 / 304，并排除可识别的机器人和验收请求。预取可能多计，缓存或离线访问可能漏计。</p><p>访客按 IP 与浏览器标识组合在内存中去重，共享网络与设备变化会影响估算。小时或每日访客数不能相加作为更长期间的去重人数。</p><p>本期全部请求中识别到 ${count(total.automated)} 条自动请求，${count(total.clientErrors)} 条 4xx、${count(total.serverErrors)} 条 5xx 响应。统计页自身请求不计入访问日志。</p><p>时间区间含开始、不含结束。页面展示最近一个双数整点之前的 24 小时，每 2 小时生成；更新失败时继续展示旧快照并提示延迟。无日志记录无法区分无人访问、离线缓存或服务器中断。</p><p>CSV 与当前屏幕来自同一份快照，仅含汇总数字，使用 UTF-8 编码，可用 Excel 打开。原始 IP、浏览器标识及完整来源网址不进入网页或下载文件。历史 CSV 中首个采集日会标记为部分采集。</p><p>原始日志沿用每日轮转、保留 10 份历史文件。每日汇总保留 400 天；服务器长时间停机、超过原始日志保留期的缺失数据无法补算。本版不包含地区、停留时间或各实验的使用热度。</p></div></details>
