...
总扫描最多100万行，包含两遍读取、空行、窗口外记录、AI及终态来源。观察最多16000个按日组合，全局分钟桶最多16000、时间样本128000、保留字符串200万字符单元。组合上限覆盖当前日志保留期而非单日人数；字符串预算和128MB服务上限仍是更强的总内存边界。每组敏感路径保留3个，页面规律采样保留按字典序前4个页面的最早64次请求；采样不足时不推断身份。原始行最多16KiB、组合标识2048、路径4096字符，保留字段复制为独立字符串，避免短切片滞留完整查询串。UA仅按有限产品token的完整边界匹配，避免设备名中的词片段误判。首遍结束后释放观察数据，只保留固定判定。超长输入、观察预算、分组上限、坏日志、坏历史、分类分区不一致均明确失败，保留旧页面。现有128MB/90秒限制不变。
